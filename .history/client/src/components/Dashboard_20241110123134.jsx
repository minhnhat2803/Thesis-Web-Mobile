...
function Dashboard() {
  ...
  const [data, setData] = useState(0); // Tổng số khách hàng
  const [totalPlatesToday, setTotalPlatesToday] = useState(0); // Số plates hiện tại
  ...
  useEffect(() => {
    // Lấy dữ liệu từ API (tổng số lượng khách hàng và plates hôm nay)
    getAllCustomer().then((res) => {
      if (res.status === 200) {
        // Đếm số lượng plates hiện tại (không null)
        const totalPlates = res.data.filter(item => item.userLicensePlate).length;
        setData(res.data.length); // Tổng số khách hàng
        setTotalPlatesToday(totalPlates); // Số plates hiện tại
      }
    });
  ...
  const cards = [
    ...
    },
    {
      ...
      data: totalPlatesToday, // Sử dụng giá trị đã tính
