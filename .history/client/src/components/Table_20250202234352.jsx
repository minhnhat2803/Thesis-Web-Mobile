...
function Table() {
  ...
  const updateParkingSlots = async (slotIDs) => {
    const promises = slotIDs.map(async (slotID) => {
      const parkingSlotDoc = doc(db, "parkingSlots", slotID);
      await updateDoc(parkingSlotDoc, {
        activity: "available",
        licensePlate: null,
      });
    });
  ...
  const deleteSelectedRows = async () => {
    ...
    await updateParkingSlots(selectedSlotIDs);
