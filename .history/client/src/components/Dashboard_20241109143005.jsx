  ...
  return (
    <div className={cx("dashboard-container")}>
      ...
      <div className={cx("dashboard-right")}>
        ...
        <div className={cx("vehicle-list")}>
          <h2>Vehicles on Site</h2>
          {data.map((vehicle) => (
            <div key={vehicle.id} className={cx("vehicle-card")}>
              <img
                src={vehicle.imageUrl}
                alt={vehicle.licensePlate}
                className={cx("vehicle-image")}
              />
              <div className={cx("vehicle-info")}>
                <p>License Plate: {vehicle.licensePlate}</p>
                <p>Entry Time: {new Date(vehicle.entryTime.seconds * 1000).toLocaleString()}</p>
                <p>Slot Number: {vehicle.slotNumber}</p>
              </div>
            </div>
          ))}
        </div>
